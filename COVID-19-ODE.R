...
kd_1 <- as.numeric(quantile(
  ger_data_deaths[50:80]/(ger_data_confirmed[40:70]), 
  probs = 0.25
))
...
f = JuliaCall::julia_eval("function f(du, u, p, t)
    ...
    # Recovered
    du[3] = + 1/ti * u[5] * (1-kh) + 1/thi * u[4] * (kh - kd) / kh
    
    # Deaths
    du[2] = + 1/thi * u[4] * kd / kh
...
JuliaCall::julia_eval("obj = build_loss_objective(prob,Tsit5(),LogLikeLoss(t,distributions),maxiters=100000,verbose=false); nothing")
...
#JuliaCall::julia_assign("t", t-t_0)
#JuliaCall::julia_assign("data", data)
#JuliaCall::julia_eval("obj = build_loss_objective(prob,Tsit5(),L2Loss(t,data), maxiters=100000,verbose=false); nothing")


JuliaCall::julia_eval("bound1 = Tuple{Float64,Float64}[(0.2,0.5),(10,20),(0.1,0.3),(20,30),(0.1,0.3)]")
JuliaCall::julia_eval("res = bboptimize(obj;SearchRange = bound1, MaxSteps = 1e5)")

p2 <- JuliaCall::julia_eval("p = best_candidate(res)")
#p2 <- c(0.2925073, 14.4655080, 0.2769363, 22.1843164, 0.1641333)
p2
rnames[p2[2]+t_0]
rnames[p2[4]+t_0]
...
for (plot_out in c(2:0)) {
  ...
  plot(c(t_0:len_ger_data)-t_0, ger_data_confirmed[c(t_0:len_ger_data),1], 
       ...
       col=1)
  points(c(t_0:len_ger_data)-t_0, ger_data_recovered[c(t_0:len_ger_data),1],col=3)
  points(c(t_0:len_ger_data)-t_0, ger_data_deaths[c(t_0:len_ger_data),1],col=2)
  ...
  points(c(t_0:len_ger_data)-t_0, ger_data_recovered[c(t_0:len_ger_data),1]/x_max*100,col=3)
  points(c(t_0:len_ger_data)-t_0, ger_data_deaths[c(t_0:len_ger_data),1]/x_max*100,col=2)
